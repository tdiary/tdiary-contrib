...
add_body_leave_proc(Proc.new do |date|
  ...
  if date > oldest_date
	 if @mode == 'day' or @mode == 'latest'
		...
		NKF.nkf('-e', mm_memo(rssurl, 50, cache_time, template_head, template_list, template_foot))
	 else
		''
	 end
  end
...
def mm_memo(url, max = 5, cache_time = 3600, \
	template_head = "<ul>\n", \
	template_list = '<li><span class="#{mm_memo_modified_class(time)}"><a href="#{CGI.escapeHTML(url)}" title="#{CGI.escapeHTML(title)}">#{CGI::escapeHTML(title)}</a></span></li>\n', \
	template_foot = "</ul>\n")
	url.untaint

	cache_file = "#{@cache_path}/rss-recent/rss-recent.#{CGI.escape(url)}"

	mm_memo_cache_rss(url, cache_file, cache_time.to_i)
	...
	i = 0
	mm_memo_read_from_cache(cache_file).each do |title, url, time, content|
		break if i >= max
...
def mm_memo_cache_rss(url, cache_file, cache_time)
	...
	if cached_time.nil? or Time.now > cached_time + cache_time
		...
		begin
			...
			rss_source = mm_memo_fetch_rss(uri, cached_time)
			...
			rss_infos = rss.items.collect do |item|
				mm_memo_pubDate_to_dc_date(item)
				[item.title, item.link, item.dc_date, item.content_encoded]
			end
			mm_memo_write_to_cache(cache_file, rss_infos)

		rescue URI::InvalidURIError
			mm_memo_write_to_cache(cache_file, [['Invalid URI', url]])
		rescue InvalidResourceError, ::RSS::Error
#			mm_memo_write_to_cache(cache_file, [['Invalid Resource', url]])
...
def mm_memo_fetch_rss(uri, cache_time)
	rss = nil
	begin
		uri.open(mm_memo_http_header(cache_time)) do |f|
			case f.status.first
			when "200"
				rss = f.read
...
def mm_memo_http_header(cache_time)
...
def mm_memo_write_to_cache(cache_file, rss_infos)
...
def mm_memo_read_from_cache(cache_file)
	require 'time'
	infos = []
	...
	infos.collect do |title, url, time, content|
		[
			mm_memo_convert(title),
			mm_memo_convert(url),
			mm_memo_convert(time) {|time| Time.parse(time)},
			mm_memo_convert(content)
		]
	end
end

def mm_memo_convert(str)
...
def mm_memo_pubDate_to_dc_date(target)
